  ...
  <Teleport to="#app">
    <div class="color-blur" ref="blurSquare">
      <svg :viewBox="`0 0 ${svgBoxSize} ${svgBoxSize}`" xmlns="http://www.w3.org/2000/svg">
        <circle v-for="(v, i) in circles" :id="`circle${i + 1}`" :key="i" :cx="v.cx" :cy="v.cy" :r="40" :fill="v.color"
          :opacity="v.opacity" />)}
      </svg>
    </div>
  </Teleport>
...
@for $i from 1 through 5 {
  ...
  #circle#{$i} {
    ...
    animation: circle#{$i}OpacityAnimation $opacityAnimationDuration cubic-bezier(0, 0, 0.58, 1) infinite,
